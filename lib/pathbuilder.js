...
function buildAdaptivePath({ selected, mastery, feedback, timeBudget, startDate }) {
  const path = buildPath({ selected, timeBudget, startDate });
  const baseWeeklyPlan = path.weeklyPlan;
  const order = path.order;
  const hw = Math.max(1, Math.min(40, timeBudget.hoursPerWeek));

  const adaptedWeeklyPlan = [];
  const seenAdapted = new Set();
  
  for (const week of baseWeeklyPlan) {
    const tid = week.topicId;
    const t = getTopic(tid);
    const m = mastery && mastery[tid];
    const theta = (m && typeof m.theta === "number") ? m.theta : 0.10;
    
    if (theta < 0.55) {
      if (!seenAdapted.has(tid)) {
        seenAdapted.add(tid);
        const mName = MILESTONE_NAMES[t.level] || `Level ${t.level}`;
        
        // 1. Fundamentals week
        adaptedWeeklyPlan.push({
          week: adaptedWeeklyPlan.length + 1,
          start: "",
          milestone: mName,
          level: t.level,
          topicId: tid,
          topic: `${t.name} Fundamentals`,
          hours: hw,
          topics: [tid],
          action: `${t.name} Fundamentals — ${hw}h. Focus on core concepts and theory.`
        });
        ...
        adaptedWeeklyPlan.push({
          ...
          level: t.level,
          topicId: tid,
          topic: `${t.name} Practice`,
          hours: hw,
          topics: [tid],
          action: `${t.name} Practice — ${hw}h. Additional exercises and hands-on practice.`
        });
        ...
        adaptedWeeklyPlan.push({
          ...
          hours: hw,
          topics: [tid],
          action: `${t.name} Quiz — ${hw}h. Retaking assessment to verify concept mastery.`
        });
      }
    } else {
      adaptedWeeklyPlan.push({
  ...
  let weekZero = start.getTime();
  adaptedWeeklyPlan.forEach((w, idx) => {
    w.week = idx + 1;
    w.start = new Date(weekZero + idx * weekMs).toISOString().slice(0, 10);
  });
  ...
  for (const tid of order) {
    ...
    if (m && m.theta < 0.55) {
      adaptiveActions.push({
        topicId: tid,
        action: "review",
        reason: `Reviewing ${getTopic(tid)?.name || tid} due to low mastery (${m.theta.toFixed(2)})`,
        mastery: m.theta
      });
    }
  }

  // Estimate total hours based on adapted plan weeks
  const totalHours = adaptedWeeklyPlan.reduce((acc, w) => acc + w.hours, 0);

  return {
    ...path,
    totalHours,
    estimatedWeeks: adaptedWeeklyPlan.length,
    weeklyPlan: adaptedWeeklyPlan,
    adaptiveActions
  };
}

module.exports = { buildPath, buildAdaptivePath, topoSort, groupMilestones, buildWeeklyPlan, MILESTONE_NAMES };